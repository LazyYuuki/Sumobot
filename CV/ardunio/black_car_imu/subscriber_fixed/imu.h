...
void setMPU6050scales(byte addr, uint8_t Gyro, uint8_t Accl) {
  ...
  Wire.endTransmission(true);
  calibrateMPU6050(addr, offsets, 'x', 5, true);
}
...
scaleddata convertRawToScaled(byte addr, rawdata data_in, bool Debug) {
  ...
  values.GyX = round((float) data_in.GyX / scale_value);
  values.GyY = round((float) data_in.GyY / scale_value);
  values.GyZ = round((float) data_in.GyZ / scale_value);
  values.timePass = millis();
