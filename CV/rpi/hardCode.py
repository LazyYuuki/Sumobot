...
while True:
    ...
    #stop condition
    if sqrt(pow((decode_list[2]-decode_list[0]), 2) + pow((decode_list[3]-decode_list[1]), 2)) > 50:
        
        move_publish(0)

    #move condition
    elif sqrt(pow((decode_list[2]-decode_list[0]), 2) + pow((decode_list[3]-decode_list[1]), 2)) < 40:
        # close to the edge of the arena
        if sqrt(pow(decode_list[0], 2) + pow(decode_list[1], 2)) > 30:
